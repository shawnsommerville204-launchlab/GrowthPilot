...
export default function AnalysisScreen({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => { const timer = window.setInterval(() => setComplete((value) => value + 1), 350); return () => window.clearInterval(timer); }, []);
  useEffect(() => { if (complete > checks.length) { const timer = window.setTimeout(onComplete, 650); return () => window.clearTimeout(timer); } }, [complete, onComplete]);
